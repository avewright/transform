#!/usr/bin/env python3
"""Harvest 16–26-piece best-first-move rows from Lichess evals.

Same incremental parquet path as the opening harvest. Does not touch
opening train. Writes READY inbox shards only.

  MOVE_VOCAB_VERSION=compact python3 -u scripts/build_lichess_middlegame_bestline.py --go
"""
...
def harvest(args: argparse.Namespace) -> None:
    ...
    log(
        f"middlegame harvest files={len(files)} pieces={MIN_PCS}-{MAX_PCS} "
    ...
    def _flush(chunk: dict) -> None:
        ...
        sh = write_inbox_shard(
            ...
            one_hot=True,
            tau=args.tau,
            meta={"min_pieces": MIN_PCS, "max_pieces": MAX_PCS, "phase": "middlegame"},
        )
        flushed += 1
    ...
    if acc:
        ...
        acc.clear()
    log(
        f"middlegame_done shards={flushed} unique={len(written):,} "
...
def main() -> None:
    ...
    ap.add_argument("--flush-every", type=int, default=80_000)
