#!/usr/bin/env python3
"""Push Lichess 16–26-piece best-line inbox shards to Hugging Face.

  python3 -u scripts/push_lichess_middlegame_bestline_hf.py --go
  python3 -u scripts/push_lichess_middlegame_bestline_hf.py --go --watch
"""
...
DEFAULT_REPO = "avewright/lichess-middlegame-bestline"
INBOX = ROOT / "outputs" / "lichess_middlegame_bestline" / "inbox"
OUT = ROOT / "outputs" / "lichess_middlegame_bestline"
...
- one-hot
- stockfish
pretty_name: Lichess 16-26 piece best-line
---

# {repo}

**{n_rows:,}** unique positions with **16 to 26 pieces**, one-hot on the
...
def convert_shard(sh: Path, dest: Path) -> int:
    ...
    if "source" not in d:
        d["source"] = torch.full((n,), 8, dtype=torch.int8)
