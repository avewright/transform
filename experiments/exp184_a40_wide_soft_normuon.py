"""exp184: A40 shallower/wider full-dim attention + soft targets + NorMuon.

Hypothesis: With only ~68 board tokens, shrinking attention head dim wastes
capacity. A shallower/wider trunk (8L/1152d) with full-dim multi-head attention
(each head keeps hidden_dim), large piece embeds (768), and soft MultiPV
targets should learn a stronger next-move policy faster than deep-narrow
stacks — especially under NorMuon on a saturated A40.

Value head: kept as a cheap 3-class WDL auxiliary (low weight). Primary goal
is next-move prediction; value is not required for that, but WDL labels are
free and stabilize representations. No RL in the main loop — soft SF MultiPV
supervision is higher Elo/$ for a 3h rental than weak self-play.
...
Data mix:
  ...
  Soft fraction of steps controlled by --soft-frac (default 0.55).

Usage:
  python experiments/exp184_a40_wide_soft_normuon.py --go --smoke
  python experiments/exp184_a40_wide_soft_normuon.py --go
  python experiments/exp184_a40_wide_soft_normuon.py --go --resume
"""
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="outputs/exp184_a40_wide_soft")
    parser.add_argument("--soft-cache", type=str, default="outputs/exp184_a40_wide_soft/soft_cache.pt")
    ...
    if not args.go:
        ...
        print("  Smoke: python experiments/exp184_a40_wide_soft_normuon.py --go --smoke")
        print("  Full:  python experiments/exp184_a40_wide_soft_normuon.py --go")
        return

    if args.smoke:
        args.steps = 80
        args.log_interval = 5
        args.save_interval = 40
        args.eval_interval = 40
        args.warmup = 10
        args.max_soft_rows = args.max_soft_rows or 8000

    model_config = DEFAULT_A40_WIDE_CONFIG
    overrides = {}
    ...
    log("=" * 64)
    log("exp184: A40 wide full-dim attn + soft MultiPV + NorMuon")
