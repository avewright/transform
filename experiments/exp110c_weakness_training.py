"""exp110c: Weakness-targeted training with hard example mining data.

Hypothesis: Training on positions where the model specifically blunders
(identified via self-play weakness harvest) will efficiently fix the model's
worst failure modes and push ELO higher than generic data augmentation.

Strategy:
  - Init from BEST checkpoint of exp110b syzygy training
  - Data: all prior sources + weakness harvest data
  - Weakness positions get 3x weight (highest value training signal)
  - Syzygy positions keep 2x weight
  - Very low LR (1e-6) since we're 3rd-stage fine-tuning
...
# ── Config ──
OUTPUT_DIR = Path("outputs/exp110c_weakness_training")
DATASET_DIRS = [
    ...
    Path("outputs/exp110_weakness_harvest/dataset"),   # weakness-mined positions
]

# This will be set dynamically at runtime (best from exp110b syzygy training)
INIT_CHECKPOINT = Path("outputs/exp110b_syzygy_training/best_model.pt")
FALLBACK_CHECKPOINT = Path("outputs/exp110_diverse_training/best_model.pt")
...
LR = 1e-6             # very low LR for 3rd-stage fine-tuning
WEIGHT_DECAY = 0.01
GRAD_CLIP = 0.5
...
SYZYGY_WEIGHT = 2.0
# Weakness-mined positions get 3x weight (highest value signal)
WEAKNESS_WEIGHT = 3.0
...
def log(msg: str):
    ...
    log_path = OUTPUT_DIR / "exp110c.log"
...
def weighted_oversample(records: list[dict]) -> list[dict]:
    """Oversample high-quality data (syzygy/tablebase/weakness) and balance phases."""
    result = []
    for r in records:
        source = r.get("source", "")
        if "weakness" in source:
            n_copies = int(WEAKNESS_WEIGHT)
            result.extend([r] * n_copies)
        elif "syzygy" in source or "tablebase" in source:
...
def main():
    ...
    log("=" * 60)
    log("exp110c: Weakness-targeted training with hard example mining")
    log("=" * 60)
