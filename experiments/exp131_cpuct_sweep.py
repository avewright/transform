"""exp131: cPUCT + noise sweep at 100 sims — the critical experiment.

CRITICAL FINDING from exp127: 200 sims at c_puct=2.5 is NOT better than
100 sims (0.500 vs 0.688). This means c_puct=2.5 wastes sims on exploration.

The AlphaZero paper uses c_puct=1.25 with 800 sims. For 100-200 sims,
lower c_puct should be even more important — we need aggressive exploitation
of the policy prior.

Additionally, Dirichlet noise at root wastes ~25% of sims on random moves.
At 100 sims, that's 25 wasted sims. Removing noise for evaluation play
should directly improve performance.

Test matrix (8 games each vs SF1900):
  Phase 1 — cPUCT sweep at 100 sims:
    cpuct_1.0          c_puct=1.0
    cpuct_1.25         c_puct=1.25 (AlphaZero original)
    cpuct_1.5          c_puct=1.5
    cpuct_2.0          c_puct=2.0

  Phase 2 — Noise ablation at best cPUCT:
    no_noise           noise_frac=0.0 (best cPUCT from Phase 1)
    low_noise          noise_frac=0.10

  Phase 3 — FPU reduction at best config:
    fpu_010            fpu_reduction=0.10
    fpu_050            fpu_reduction=0.50

  Phase 4 — Combined best config at 100 AND 200 sims:
    best_100           best cPUCT + no noise + best FPU
    best_200           same config at 200 sims
...
def main():
    ...
    ap.add_argument("--phase", type=int, default=0,
                    help="0=all 4 phases, 1=cpuct, 2=noise, 3=fpu, 4=combined")
    # Allow overriding best config for phases 2-4
    ap.add_argument("--best-cpuct", type=float, default=1.25,
                    help="Best cPUCT from Phase 1 (default: 1.25 = AlphaZero)")
    ap.add_argument("--best-fpu", type=float, default=0.25)
    ap.add_argument("--best-noise", type=float, default=0.0)
    args = ap.parse_args()
    ...
    global LOG_PATH
    LOG_PATH = ROOT / "outputs" / "exp131_cpuct_sweep.log"
    json_path = ROOT / "outputs" / "exp131_cpuct_sweep.json"
    ...
    if args.phase in (0, 1):
        log("\n" + "=" * 60)
        log("PHASE 1: cPUCT SWEEP AT 100 SIMS")
        log("=" * 60)

        for cpuct in [1.0, 1.25, 1.5, 2.0]:
            r = run_config(model, syzygy, sf_elo, n_games, 100,
                           label=f"cpuct_{cpuct}",
    ...
    if args.phase in (0, 2):
        log("\n" + "=" * 60)
        log(f"PHASE 2: NOISE ABLATION (c={args.best_cpuct})")
        log("=" * 60)

        for nf, label in [(0.0, "no_noise"), (0.10, "low_noise_10")]:
            r = run_config(model, syzygy, sf_elo, n_games, 100,
                           label=label,
                           c_puct=args.best_cpuct, noise_frac=nf)
            all_results.append(r)

    # Phase 3: FPU reduction at best config
    if args.phase in (0, 3):
        log("\n" + "=" * 60)
        log(f"PHASE 3: FPU SWEEP (c={args.best_cpuct}, noise={args.best_noise})")
        log("=" * 60)

        for fpu, label in [(0.10, "fpu_010"), (0.50, "fpu_050")]:
            r = run_config(model, syzygy, sf_elo, n_games, 100,
                           label=label,
                           c_puct=args.best_cpuct,
                           noise_frac=args.best_noise,
                           fpu_reduction=fpu)
            all_results.append(r)

    # Phase 4: Combined best at 100 and 200 sims
    if args.phase in (0, 4):
        log("\n" + "=" * 60)
        log(f"PHASE 4: BEST CONFIG AT 100 + 200 SIMS")
        log("=" * 60)
    ...
    log("\nReferences:")
    log("  exp125 fixed_100 (c=2.5): 0.688 → ELO ~2037")
    log("  exp127 fixed_200 (c=2.5): ~0.50 → ELO ~1900 (worse!)")
