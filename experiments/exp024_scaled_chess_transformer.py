"""exp024: Scale Chess Transformer to full HF dataset (475K positions).

Hypothesis: The chess transformer from exp023 reached 40.5% accuracy on 50K data
with loss still dropping at epoch 10 — it's severely data-starved. Training on
the full 475K dataset should unlock significantly higher accuracy.

Background:
  - exp023: chess transformer 8L/512d, 50K data → 40.5% best acc, W0/D0/L8
  - Loss was still declining at epoch 10 (not converged)
  - With frozen Qwen3, 50K→200K gave no improvement (36.5% both)
  - Chess transformer has 26M trainable params → needs more data than frozen approach

Changes from exp023:
  - Use ALL available HF data (~460K after validation)
  - 3 epochs (more data, fewer passes to avoid overfitting)
  - Larger batch size (256) for stability
...
OUTPUT_DIR = Path("outputs/exp024_scaled_chess_transformer")
...
def main():
    ...
    # Use last 2K for eval, rest for training
    ...
    # --- Train ---
    print(f"\n[3/3] Training on {len(train_data)} positions, {EPOCHS} epochs (batch={BATCH_SIZE}x{ACCUM_STEPS})")
    history = []
    best_acc = 0
    global_step = 0
    ...
    print(f"\n  Loaded best checkpoint (epoch {ckpt['epoch']}, acc={ckpt['accuracy']:.1%})")
    ...
    results = {
        "experiment": "exp024_scaled_chess_transformer",
        "hypothesis": "Full dataset unlocks higher accuracy for chess transformer",
        "seed": SEED,
    ...
    print(f"  Train data: {len(train_data):,} (full dataset)")
    print(f"  Trainable: {trainable:,}")
