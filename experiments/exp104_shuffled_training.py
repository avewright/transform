"""exp104: Cross-file shuffled training with label smoothing.

Hypothesis: Training on data shuffled across many parquet files (instead of
processing files sequentially) will prevent the distribution-shift degradation
observed in exp101/exp103. Label smoothing will further improve generalization.

Key differences from exp101:
  - Pre-loads N parquet files into CPU RAM, concatenates and shuffles uniformly
  - Label smoothing (legal-move-aware) instead of hard CE
  - 10K eval set for reliable measurement
  - torch.compile for ~20% throughput boost

Architecture: Same ChessTransformer200M as exp101

Evidence:
  - exp101 v2 achieved 17.26% acc (10K eval) but degraded after step ~900
  - Distribution shift between sequential parquet files is the likely cause
  - 503GB RAM available → can load 100+ files (~25M+ positions) into CPU RAM
"""
...
OUTPUT_DIR = Path("/root/transform/outputs/exp104_shuffled")
...
def main():
    ...
    LOG_FILE = open(out_dir / "exp104.log", "a")

    log("=" * 60)
    log("exp104: Cross-file shuffled training with label smoothing")
