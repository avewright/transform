"""exp083: Full-corpus pretraining on 4xA40 with aggressive logging & checkpointing.

Hypothesis: Training the 204M ChessTransformer on the full ~832M position corpus
using 4 A40 GPUs with Local SGD will produce a significantly stronger model.

Architecture: ChessTransformer200M (identical to exp073/074/075)
...
Strategy:
  ...
  - Download best_model.pt from avewright/chess-transformer-200m-v2 as init
  - Every SYNC_INTERVAL optimizer steps, workers average weights
  - Batch 256, gradient accumulation 4 (effective 1024 per worker)
  - Cosine LR 1e-4 → 5% floor with 1% warmup

Logging & health:
  - Per-worker JSONL event logs (every step metric)
  - Heartbeat every 10 steps
  - Eval every 500 steps (worker 0)
  - Checkpoint every 100 optimizer steps
  - Global training_log.json + per-worker events
  - GPU memory, temperature, throughput tracked
  - Graceful shutdown on SIGINT/SIGTERM

GPU: 4× NVIDIA A40 46GB

Usage:
  python experiments/exp083_pretrain_4xa40.py
"""
...
def _train_worker_inner(worker_id, device, worker_dir):
    ...
    print(f"\n{'='*72}")
    print(f" EXP083 WORKER {worker_id} on GPU {worker_id} ({gpu_name})")
    print(f"{'='*72}")
    ...
    if own_ckpt is not None:
        ...
        # Initialize from HF pretrained model
...
def train():
    ...
    print(f"\n{'='*72}")
    print(f" EXP083: FULL-CORPUS PRETRAINING — {NUM_GPUS}×A40")
    print(f"{'='*72}")
    print(f"  Timestamp:  {utcnow_iso()}")
    ...
    atomic_write_json(
        ...
        {
            "timestamp": utcnow_iso(),
            "experiment": "exp083_pretrain_4xa40",
            "num_gpus": NUM_GPUS,
