"""exp083c: Safer continuation from exp083 best checkpoint.
...
destroyed learned representations. exp083b lowered LR to 3e-5 but still looked
mostly flat on the fixed eval set. This run takes the safer path: continue from
the exp083 step-500 best model (16.7% accuracy) with LR=1e-5.
...
Changes from exp083:
  - LR: 1e-4 → 1e-5 (very conservative continuation)
  - WARMUP_FRAC: 0.01 → 0.002 (tiny warmup since model already good)
  - Init from: exp083 best_model.pt (step 500, 16.7% acc) instead of HF
  - MIN_LR_FRAC: 0.05 → 0.20 (higher floor to keep updates alive but gentle)
...
Strategy:
  ...
  - Cosine LR 1e-5 → 20% floor with 0.2% warmup

GPU: 4× NVIDIA A40 46GB

Usage:
  python experiments/exp083c_pretrain_lr1e5.py
"""
...
# ── Paths ──
OUTPUT_DIR = Path("outputs/exp083c_pretrain_lr1e5")
...
KEEP_CHECKPOINTS = 2     # per worker (2 × 4 × 2.4GB ≈ 19GB max)

# Early-stop guard for expensive continuation runs.
...
def request_stop_run(reason: str, **fields):
    atomic_write_json(
        OUTPUT_DIR / "stop_training.json",
        {"timestamp": utcnow_iso(), "reason": reason, **fields},
    )


def stop_requested_by_file() -> bool:
    return (OUTPUT_DIR / "stop_training.json").exists()
...
def evaluate(model, eval_data, eval_tensors, device, batch_size=128):
    ...
    with torch.no_grad():
        for i in range(0, len(eval_data), batch_size):
            ...
            for j, d in enumerate(chunk):
                ...
                entropy = -(probs[mask] * log_probs[mask]).sum().item() if legal_count > 0 else 0.0
                true_prob_sum += true_prob
                true_nll_sum += -log_probs[true_idx].item()
                ...
                phase_stats[phase]["true_prob_sum"] += true_prob
                phase_stats[phase]["true_nll_sum"] += -log_probs[true_idx].item()
            ...
            for j, d in enumerate(chunk):
                ...
                if pred_class == true_class:
                    val_correct += 1
                val_total += 1
                target = torch.tensor(true_wdl, device=wdl_logits.device, dtype=wdl_logits.dtype)
                value_kl_sum += F.kl_div(
                    F.log_softmax(wdl_logits[j], dim=-1), target, reduction="sum"
                ).item()
    ...
    phase_true_prob = {
        p: round(s["true_prob_sum"] / max(s["total"], 1), 4)
        for p, s in phase_stats.items()
    }
    phase_true_nll = {
        p: round(s["true_nll_sum"] / max(s["total"], 1), 4)
        for p, s in phase_stats.items()
    }
    return {
        ...
        "avg_true_move_prob": true_prob_sum / max(total, 1),
        "avg_true_move_nll": true_nll_sum / max(total, 1),
        "avg_pred_confidence": pred_conf_sum / max(total, 1),
        "avg_legal_entropy": entropy_sum / max(total, 1),
        "value_accuracy": val_correct / max(val_total, 1),
        "value_kl": value_kl_sum / max(val_total, 1),
        "phase_accuracy": phase_accuracy,
        "phase_true_move_prob": phase_true_prob,
        "phase_true_move_nll": phase_true_nll,
        "n_eval": total,
    }
...
def _train_worker_inner(worker_id, device, worker_dir):
    ...
    print(f"\n{'='*72}")
    print(f" EXP083c WORKER {worker_id} on GPU {worker_id} ({gpu_name})")
    print(f"{'='*72}")
    ...
    if worker_id == 0 and global_step == 0:
        ...
        print(f"  Loaded model: acc={ev0['accuracy']:.1%} "
              ...
              f"true_p={ev0['avg_true_move_prob']:.3f} "
              f"nll={ev0['avg_true_move_nll']:.3f} "
              f"val={ev0['value_accuracy']:.1%} "
              f"vkl={ev0['value_kl']:.3f}")
        ...
        if micro_step % ACCUM_STEPS == 0:
            ...
            if worker_id == 0 and global_step % EVAL_INTERVAL == 0:
                ...
                print(f"\n    [W0] ** EVAL step {global_step}: "
                      ...
                      f"true_p={ev['avg_true_move_prob']:.3f} "
                      f"nll={ev['avg_true_move_nll']:.3f} "
                      f"val={ev['value_accuracy']:.1%} "
                      f"vkl={ev['value_kl']:.3f} "
                ...
                append_metrics(0, {
                    ...
                    "avg_true_move_prob": round(ev["avg_true_move_prob"], 4),
                    "avg_true_move_nll": round(ev["avg_true_move_nll"], 4),
                    "avg_pred_confidence": round(ev["avg_pred_confidence"], 4),
                    "avg_legal_entropy": round(ev["avg_legal_entropy"], 4),
                    "value_accuracy": round(ev["value_accuracy"], 4),
                    "value_kl": round(ev["value_kl"], 4),
                    "phase_accuracy": ev["phase_accuracy"],
                    "phase_true_move_prob": ev["phase_true_move_prob"],
                    "phase_true_move_nll": ev["phase_true_move_nll"],
                })
...
def train():
    ...
    print(f"\n{'='*72}")
    print(f" EXP083c: SAFER CONTINUATION (LR=1e-5) FROM EXP083 BEST — {NUM_GPUS}×A40")
    print(f"{'='*72}")
    print(f"  Timestamp:  {utcnow_iso()}")
    ...
    atomic_write_json(
        ...
        {
            "timestamp": utcnow_iso(),
            "experiment": "exp083c_pretrain_lr1e5",
