#!/usr/bin/env python3
"""exp276: 99M FT on streaming Lichess <14-piece best-line (one-hot).

Harvest writes READY inbox shards. This packs a frozen hash-holdout val,
then trains in short chunks while absorbing new shards into train.

  MOVE_VOCAB_VERSION=compact python experiments/exp276_lichess_endgame_stream.py --go
"""
...
OUT_DIR = ROOT / "outputs" / "exp276_lichess_endgame_stream"
...
def tag_rows(table: dict) -> dict:
    ...
    table = dict(table)
    table["source"] = torch.full((n,), SOURCE_LICHESS, dtype=torch.int8)
...
def reload_train_capped(out: Path, inbox: Path, *, cap: int) -> int:
    """Rebuild train.pt from newest inbox shards, dropping frozen val hashes."""
    from autoresearch_8gb.pipeline import concat_soft_tables, filter_disjoint, position_hashes
...
def harvest_alive() -> bool:
    ...
    r = subprocess.run(["pgrep", "-af", "build_lichess"], capture_output=True, text=True)
    return any("build_lichess" in ln and "cursor" not in ln for ln in (r.stdout or "").splitlines())
...
def trial_config() -> dict:
    ...
    return {
        "id": "exp276_lichess_endgame_stream",
        "arch": "squares64",
        "desc": "99M FT on streaming Lichess <14 best-line one-hot, frozen stratified val.",
...
def train_loop(args: argparse.Namespace) -> None:
    ...
    cfg["external_eval"] = {"lichess_endgame": str((out / "lichess_eval.pt").resolve())}
...
def main() -> None:
    ...
    _assert_compact()
    print(
        f"exp276 99M Lichess <14 one-hot stream  split={TRAIN_PCT}/{100 - TRAIN_PCT} val_n={args.val_n}",
        flush=True,
    )
