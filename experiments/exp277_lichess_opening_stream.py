#!/usr/bin/env python3
"""exp277: 99M FT on streaming Lichess >=26-piece best-line (one-hot).
...
Upload target: avewright/opening-model.

  MOVE_VOCAB_VERSION=compact python experiments/exp277_lichess_opening_stream.py --go
"""
...
OUT_DIR = ROOT / "outputs" / "exp277_lichess_opening_stream"
...
def tag_rows(table: dict) -> dict:
    ...
    table = dict(table)
    table["source"] = torch.full((n,), SOURCE_LICHESS_OPENING, dtype=torch.int8)
...
def pack_initial(out: Path, inbox: Path, *, val_n: int = VAL_N) -> dict:
    ...
    report = {
        ...
        "hf_model": "avewright/opening-model",
        "hf_data": "avewright/lichess-opening-bestline",
    }
...
def harvest_alive() -> bool:
    ...
    r = subprocess.run(["pgrep", "-af", "build_lichess_opening_bestline"], capture_output=True, text=True)
    return any("build_lichess_opening_bestline" in ln and "cursor" not in ln for ln in (r.stdout or "").splitlines())
...
def trial_config() -> dict:
    ...
    return {
        "id": "exp277_lichess_opening_stream",
        "arch": "squares64",
        "desc": "99M FT on streaming Lichess >=26 best-line one-hot, frozen stratified val.",
...
def train_loop(args: argparse.Namespace) -> None:
    ...
    cfg["external_eval"] = {"lichess_opening": str((out / "lichess_eval.pt").resolve())}
...
def main() -> None:
    ...
    _assert_compact()
    print(
        f"exp277 99M Lichess >=26 one-hot stream  split={TRAIN_PCT}/{100 - TRAIN_PCT} val_n={args.val_n} "
        f"→ avewright/opening-model",
        flush=True,
    )
