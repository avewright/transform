"""exp031: Extended training of chess transformer on full 460K dataset, 6 epochs.

Hypothesis: Training on 460K data for 6 epochs (vs exp024's 3 epochs) will
significantly improve accuracy because exp024's loss was still declining at
epoch 3 (2.33) with no signs of overfitting.

Background:
  - exp024: 8L/512d, 460K, 3ep → 48.7%, loss=2.33 (still declining)
  - exp023: 8L/512d, 50K, 10ep → 40.5%
  - Five consecutive 50K ablations (exp026-030) ALL produced TIEs
  - The only proven lever is total gradient volume (data × epochs)
  - exp029 showed diversity vs repetition is neutral at matched compute →
    simply training longer should help

Design:
  - Same architecture as exp024 (8L/512d/8h chess transformer)
  - Full 460K HF dataset
  - 6 epochs (doubling exp024's 3 epochs)
  - Cosine LR with warmup 500 steps
  - Gradient accumulation (128 × 2 = 256 effective batch)
  - Save best checkpoint

Primary metric: top-1 accuracy, games vs SF d3
Time budget: ~2.5 hours (this exceeds the 10-min guideline but is the
highest-value experiment available — all quick ablations are exhausted)
"""
...
OUTPUT_DIR = Path("outputs/exp031_extended_training")
...
def main():
    ...
    print(f"Experiment: exp031_extended_training")
    print(f"Hypothesis: 6 epochs on 460K > 3 epochs (exp024 baseline)")

    # --- Load data ---
    print("\n[1/3] Loading FULL HuggingFace dataset...")
    ...
    # --- Build model ---
    print(f"\n[2/3] Building Chess Transformer (8L/512d/8h)")
    ...
    print(f"\n[3/3] Training on {len(train_data):,} positions, {EPOCHS} epochs "
          f"(batch={BATCH_SIZE}×{ACCUM_STEPS})")
    history = []
    best_acc = 0
    global_step = 0
    ...
    for epoch in range(EPOCHS):
        ...
        if ev["accuracy"] > best_acc:
            ...
            torch.save({
                ...
                "epoch": epoch + 1, "accuracy": ev["accuracy"],
            }, OUTPUT_DIR / "best_checkpoint.pt")

        history.append({**ev, "loss": round(avg_loss, 4), "epoch": epoch + 1})
    ...
    print(f"\n  Best: {best_acc:.1%}  Final: {final['accuracy']:.1%}")
    print(f"  vs exp024 (3ep): 48.7%")

    # --- Load best & play games ---
    ckpt = torch.load(OUTPUT_DIR / "best_checkpoint.pt", map_location=device, weights_only=True)
    model.load_state_dict(ckpt["model_state"])
    print(f"  Loaded best checkpoint (epoch {ckpt['epoch']}, acc={ckpt['accuracy']:.1%})")
    ...
    results = {
        "experiment": "exp031_extended_training",
        "hypothesis": "6 epochs on 460K data > 3 epochs (exp024, loss still declining at 2.33)",
        ...
        "training": {"epochs": EPOCHS, "batch_size": BATCH_SIZE,
                     ...
                     "lr": LR, "warmup_steps": WARMUP_STEPS, "schedule": "cosine"},
        "results": {
            "best_acc": best_acc,
            "final": final,
            "history": history,
        ...
        "device": str(device),
        "command": "python experiments/exp031_extended_training.py",
    }
    with open(OUTPUT_DIR / "results.json", "w") as f:
        json.dump(results, f, indent=2)

    print(f"\n{'='*60}")
    print(f" SUMMARY: exp031_extended_training")
    print(f" Hypothesis: 6ep on 460K > 3ep (exp024)")
    print(f" Best accuracy: {best_acc:.1%} (exp024: 48.7%)")
    print(f" Delta vs exp024: {best_acc - 0.487:+.1%}")
