"""exp169: Micro-scale architecture ablation — rapid hypothesis testing.

Goal: Test architectural features head-to-head with a small model (~3M params)
to identify which changes deliver the most signal before scaling to 204M on A40.

Each ablation trains for 3000 steps on shard_00000 (1M positions) with:
  - 4L / 256d / 4H = ~3M params
  - Effective batch size 96 (bs=96, no accum needed at this scale)
  - ~500+ pos/s on RTX 4060 → ~10 min per run
  - Eval every 500 steps on first 5000 eval positions

Ablations:
  A. BASELINE      — Vanilla transformer (GELU, abs pos embed, SpatialPolicyHead)
  B. SWIGLU        — Replace GELU FFN with SwiGLU gated activation (2/3 FFN dim)
  C. REL_BIAS      — Add chess-aware relative geometry bias to attention
  D. ATTN_POLICY   — Replace SpatialPolicyHead with attention-based policy head
  E. COMBINED      — SwiGLU + RelBias + AttnPolicy all together

Metrics at step 3000: policy_loss, top-1 accuracy, top-3 accuracy
The RELATIVE ordering across ablations is what matters, not absolute numbers.

Usage:
  python experiments/exp169_micro_ablation.py              # Run ALL ablations
  python experiments/exp169_micro_ablation.py --variant B  # Run only SwiGLU
  python experiments/exp169_micro_ablation.py --steps 1500 # Shorter runs
"""

import argparse
import gc
import json
import math
import os
import sys
import time
from datetime import datetime
from pathlib import Path

os.environ['PYTHONUNBUFFERED'] = '1'
os.environ['MOVE_VOCAB_VERSION'] = 'compact'

import torch
import torch.nn as nn
import torch.nn.functional as F
from torch.optim import AdamW
from torch.amp import autocast, GradScaler

torch.backends.cuda.matmul.allow_tf32 = True
torch.backends.cudnn.allow_tf32 = True
torch.backends.cudnn.benchmark = True

sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

from chess_transformer_factory import ChessTransformerConfig, SpatialPolicyHead, _build_move_square_indices
from chess_model import FusedBoardEncoder
from move_vocab import (
    VOCAB_SIZE, IDX_TO_UCI, LEGACY_UCI_TO_IDX, legacy_to_compact_map,
)
from data_loader import board_array_to_fused, ep_square_to_file, compute_wdl

import chess

ROOT = Path(__file__).resolve().parent.parent
SHARD_DIR = ROOT / "outputs" / "exp139_massive_train" / "shards"
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# ── Compact vocab remap ──
def build_remap_tensor():
    remap = legacy_to_compact_map()
    legacy_size = max(LEGACY_UCI_TO_IDX.values()) + 1
    t = torch.full((legacy_size,), -1, dtype=torch.long)
    for old_idx, new_idx in remap.items():
        t[old_idx] = new_idx
    return t

REMAP = build_remap_tensor()

# ── SwiGLU FFN ──
class SwiGLUFFN(nn.Module):
    """SwiGLU gated feedforward: W1(x)*sigmoid(W1(x)) * W2(x), then W3 down-project."""
    def __init__(self, d_model, ffn_dim, dropout=0.1):
        super().__init__()
        # SwiGLU uses 2/3 of ffn_dim for each gate to keep param count similar
        inner = int(ffn_dim * 2 / 3)
        self.w_gate = nn.Linear(d_model, inner)
        self.w_up = nn.Linear(d_model, inner)
        self.w_down = nn.Linear(inner, d_model)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        return self.dropout(self.w_down(F.silu(self.w_gate(x)) * self.w_up(x)))


# ── Chess Relative Bias ──
class ChessRelBias(nn.Module):
    """Lightweight chess geometry bias for attention. Adds per-head learned bias."""
    def __init__(self, num_heads, n_ctx=5):  # CLS + 4 context tokens
        super().__init__()
        self.num_heads = num_heads
        self.n_ctx = n_ctx
        seq_len = n_ctx + 64

        # Precompute geometry features for 64x64 square pairs
        rank_dist = torch.zeros(64, 64, dtype=torch.long)
        file_dist = torch.zeros(64, 64, dtype=torch.long)
        same_diag = torch.zeros(64, 64, dtype=torch.long)
        knight_rel = torch.zeros(64, 64, dtype=torch.long)

        for i in range(64):
            ri, fi = i // 8, i % 8
            for j in range(64):
                rj, fj = j // 8, j % 8
                dr, df = abs(ri - rj), abs(fi - fj)
                rank_dist[i, j] = dr
                file_dist[i, j] = df
                same_diag[i, j] = int(dr == df and dr > 0)
                knight_rel[i, j] = int(sorted([dr, df]) == [1, 2])

        # Learned bias per feature bucket per head
        self.rank_bias = nn.Embedding(8, num_heads)
        self.file_bias = nn.Embedding(8, num_heads)
        self.diag_bias = nn.Embedding(2, num_heads)
        self.knight_bias = nn.Embedding(2, num_heads)

        # Bias for context tokens
        self.ctx_bias = nn.Parameter(torch.zeros(num_heads, seq_len, seq_len))

        self.register_buffer("rank_dist", rank_dist)
        self.register_buffer("file_dist", file_dist)
        self.register_buffer("same_diag", same_diag)
        self.register_buffer("knight_rel", knight_rel)

    def forward(self, seq_len):
        nc = self.n_ctx
        bias = self.ctx_bias[:, :seq_len, :seq_len].clone()

        # Square-to-square geometry biases
        rb = self.rank_bias(self.rank_dist).permute(2, 0, 1)  # (H, 64, 64)
        fb = self.file_bias(self.file_dist).permute(2, 0, 1)
        db = self.diag_bias(self.same_diag).permute(2, 0, 1)
        kb = self.knight_bias(self.knight_rel).permute(2, 0, 1)

        bias[:, nc:nc+64, nc:nc+64] = rb + fb + db + kb
        return bias


# ── Attention Policy Head ──
class AttentionPolicyHead(nn.Module):
    """Scaled dot-product attention policy head (ChessFormer-style)."""
    def __init__(self, hidden_dim, n_ctx_tokens=5, num_heads=4, head_dim=64):
        super().__init__()
        self.n_ctx = n_ctx_tokens
        self.num_heads = num_heads
        self.head_dim = head_dim
        self.q_proj = nn.Linear(hidden_dim, num_heads * head_dim, bias=False)
        self.k_proj = nn.Linear(hidden_dim, num_heads * head_dim, bias=False)
        self.gate = nn.Sequential(
            nn.Linear(hidden_dim, num_heads),
            nn.Sigmoid(),
        )
        self.promo_embed = nn.Embedding(5, 1)  # additive bias for promotion moves
        from_sqs, to_sqs, promo_types = _build_move_square_indices()
        self.register_buffer("from_sqs", from_sqs)
        self.register_buffer("to_sqs", to_sqs)
        self.register_buffer("promo_types", promo_types)
        self.scale = head_dim ** -0.5

    def forward(self, hidden_states, cls_hidden):
        B = hidden_states.shape[0]
        sq_hidden = hidden_states[:, self.n_ctx:self.n_ctx + 64, :]

        Q = self.q_proj(sq_hidden).view(B, 64, self.num_heads, self.head_dim)
        K = self.k_proj(sq_hidden).view(B, 64, self.num_heads, self.head_dim)

        # Gather per move
        q_from = Q[:, self.from_sqs, :, :]  # (B, V, H, D)
        k_to = K[:, self.to_sqs, :, :]      # (B, V, H, D)

        # Scaled dot product per head
        scores = (q_from * k_to).sum(-1) * self.scale  # (B, V, H)

        # Gate heads by CLS
        g = self.gate(cls_hidden).unsqueeze(1)  # (B, 1, H)
        scores = (scores * g).sum(-1)  # (B, V)

        # Add promotion bias
        scores = scores + self.promo_embed(self.promo_types).squeeze(-1).unsqueeze(0)

        return scores


# ── Custom TransformerEncoderLayer with SwiGLU ──
class SwiGLUTransformerLayer(nn.Module):
    """Pre-norm transformer layer with SwiGLU FFN instead of GELU."""
    def __init__(self, d_model, nhead, ffn_dim, dropout=0.1):
        super().__init__()
        self.norm1 = nn.LayerNorm(d_model)
        self.attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout, batch_first=True)
        self.norm2 = nn.LayerNorm(d_model)
        self.ffn = SwiGLUFFN(d_model, ffn_dim, dropout)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x, attn_bias=None):
        # Pre-norm self-attention
        normed = self.norm1(x)
        attn_out, _ = self.attn(normed, normed, normed, attn_mask=attn_bias)
        x = x + self.dropout(attn_out)
        # Pre-norm SwiGLU FFN
        x = x + self.ffn(self.norm2(x))
        return x


# ── Micro Model Builder ──
class MicroChessTransformer(nn.Module):
    """Configurable micro model for ablation testing."""
    def __init__(self, d_model=256, n_layers=4, n_heads=4, ffn_ratio=4,
                 dropout=0.1, policy_head_dim=128,
                 use_swiglu=False, use_rel_bias=False, use_attn_policy=False):
        super().__init__()
        self.d_model = d_model
        self.n_ctx = 4  # CLS + turn + castling + ep = 4 tokens before squares
        n_ctx_plus_cls = 4  # squares start at index 4

        # Encoder
        self.encoder = FusedBoardEncoder(embed_dim=d_model)
        # No input_proj needed since encoder_dim == hidden_dim for micro models

        # CLS token
        self.cls_token = nn.Parameter(torch.randn(1, 1, d_model) * 0.02)

        # Positional embedding
        self.pos_embed = nn.Parameter(torch.randn(1, 68, d_model) * 0.02)

        # Relative bias
        self.rel_bias = ChessRelBias(n_heads, n_ctx=4) if use_rel_bias else None

        # Transformer layers
        ffn_dim = d_model * ffn_ratio
        if use_swiglu:
            self.layers = nn.ModuleList([
                SwiGLUTransformerLayer(d_model, n_heads, ffn_dim, dropout)
                for _ in range(n_layers)
            ])
            self._custom_layers = True
        else:
            enc_layer = nn.TransformerEncoderLayer(
                d_model=d_model, nhead=n_heads,
                dim_feedforward=ffn_dim, dropout=dropout,
                activation="gelu", batch_first=True, norm_first=True,
            )
            self.transformer = nn.TransformerEncoder(enc_layer, num_layers=n_layers)
            self._custom_layers = False

        self.norm = nn.LayerNorm(d_model)

        # Policy head
        if use_attn_policy:
            self.policy_head = AttentionPolicyHead(
                d_model, n_ctx_tokens=4,
                num_heads=n_heads, head_dim=policy_head_dim // n_heads,
            )
        else:
            self.policy_head = SpatialPolicyHead(
                d_model, n_ctx_tokens=4, head_dim=policy_head_dim,
            )

        # Simple value head (3-class WDL for speed)
        self.value_head = nn.Sequential(
            nn.Linear(d_model, d_model // 2),
            nn.ReLU(),
            nn.Linear(d_model // 2, 3),
        )

        self._use_swiglu = use_swiglu
        self._use_rel_bias = use_rel_bias
        self._use_attn_policy = use_attn_policy

    def forward(self, board_input):
        hidden = self.encoder(board_input)  # (B, 67, d_model)
        B = hidden.shape[0]

        # Prepend CLS
        hidden = torch.cat([self.cls_token.expand(B, -1, -1), hidden], dim=1)  # (B, 68, d)

        # Position embed
        hidden = hidden + self.pos_embed

        if self._custom_layers:
            bias = self.rel_bias(hidden.shape[1]) if self.rel_bias else None
            if bias is not None:
                # Expand for batch: (H, S, S) → (B*H, S, S)
                bias = bias.unsqueeze(0).expand(B, -1, -1, -1).reshape(
                    B * self.layers[0].attn.num_heads, hidden.shape[1], hidden.shape[1])
            for layer in self.layers:
                hidden = layer(hidden, attn_bias=bias)
        else:
            if self.rel_bias:
                bias = self.rel_bias(hidden.shape[1])
                # Need to expand for nn.TransformerEncoder format
                n_heads = self.transformer.layers[0].self_attn.num_heads
                bias = bias.unsqueeze(0).expand(B, -1, -1, -1).reshape(
                    B * n_heads, hidden.shape[1], hidden.shape[1])
                # nn.TransformerEncoder doesn't easily accept custom masks per-layer
                # Use the custom layer path for rel_bias instead
                raise RuntimeError("Use use_swiglu=True with rel_bias or custom layers")
            hidden = self.transformer(hidden)

        hidden = self.norm(hidden)
        cls_hidden = hidden[:, 0, :]

        return {
            "policy_logits": self.policy_head(hidden, cls_hidden),
            "value_logits": self.value_head(cls_hidden),
        }


# ── Data Loading ──
def load_shard(shard_idx=0):
    """Load a single training shard into memory."""
    path = SHARD_DIR / f"shard_{shard_idx:05d}.pt"
    data = torch.load(path, weights_only=True, map_location="cpu")
    return data


def prepare_batch(data, indices, device):
    """Prepare a batch from pre-loaded shard data."""
    ba = data["board_array"][indices]
    turn = data["turn"][indices]
    castling = data["castling"][indices]
    ep = data["ep_square"][indices]
    move_idx = data["move_idx"][indices]
    cp = data["cp"][indices]
    mate = data["mate"][indices]

    fused_ids = board_array_to_fused(ba)
    ep_file = ep_square_to_file(ep)

    # Remap legacy move indices to compact
    move_compact = REMAP[move_idx.long()]
    valid = move_compact >= 0

    # WDL targets
    wdl = compute_wdl(cp.float(), mate.float())

    board_input = {
        "fused_ids": fused_ids.to(device),
        "turn": turn.long().to(device),
        "castling": castling.long().to(device),
        "ep_file": ep_file.long().to(device),
    }

    return board_input, move_compact[valid].to(device), wdl[valid].to(device), valid


def load_eval_subset(n=5000):
    """Load eval data from cached eval set or first N of shard 1."""
    eval_cache = ROOT / "outputs" / "data_cache" / "eval_5k.pt"
    if eval_cache.exists():
        d = torch.load(eval_cache, weights_only=True, map_location="cpu")
        return d
    # Fallback: use start of shard 1
    return load_shard(1)


# ── Evaluation ──
@torch.no_grad()
def evaluate(model, eval_data, device, n_eval=5000):
    model.eval()
    ba = eval_data["board_array"][:n_eval]
    turn = eval_data["turn"][:n_eval]
    castling = eval_data["castling"][:n_eval]
    ep = eval_data["ep_square"][:n_eval]
    move_idx = eval_data["move_idx"][:n_eval]

    fused_ids = board_array_to_fused(ba)
    ep_file = ep_square_to_file(ep)
    move_compact = REMAP[move_idx.long()]
    valid = move_compact >= 0

    total_correct_1 = 0
    total_correct_3 = 0
    total_loss = 0.0
    total_n = 0

    bs = 256
    for start in range(0, valid.sum().item(), bs):
        end = min(start + bs, valid.sum().item())
        # Get valid indices
        valid_indices = valid.nonzero(as_tuple=True)[0][start:end]
        bi = {
            "fused_ids": fused_ids[valid_indices].to(device),
            "turn": turn[valid_indices].long().to(device),
            "castling": castling[valid_indices].long().to(device),
            "ep_file": ep_file[valid_indices].long().to(device),
        }
        targets = move_compact[valid_indices].to(device)

        with autocast("cuda", dtype=torch.float16):
            out = model(bi)
        logits = out["policy_logits"].float()

        loss = F.cross_entropy(logits, targets, reduction="sum")
        total_loss += loss.item()

        _, top3 = logits.topk(3, dim=-1)
        total_correct_1 += (top3[:, 0] == targets).sum().item()
        total_correct_3 += (top3 == targets.unsqueeze(1)).any(dim=1).sum().item()
        total_n += targets.shape[0]

    model.train()
    return {
        "loss": total_loss / max(total_n, 1),
        "top1": total_correct_1 / max(total_n, 1),
        "top3": total_correct_3 / max(total_n, 1),
        "n": total_n,
    }


# ── Ablation Variants ──
VARIANTS = {
    "A": {"name": "BASELINE",  "use_swiglu": False, "use_rel_bias": False, "use_attn_policy": False},
    "B": {"name": "SWIGLU",    "use_swiglu": True,  "use_rel_bias": False, "use_attn_policy": False},
    "C": {"name": "REL_BIAS",  "use_swiglu": True,  "use_rel_bias": True,  "use_attn_policy": False},
    "D": {"name": "ATTN_POL",  "use_swiglu": False, "use_rel_bias": False, "use_attn_policy": True},
    "E": {"name": "COMBINED",  "use_swiglu": True,  "use_rel_bias": True,  "use_attn_policy": True},
}


def train_variant(variant_key, max_steps=3000, eval_every=500, batch_size=96,
                  lr=3e-4, seed=42):
    """Train one ablation variant and return results."""
    cfg = VARIANTS[variant_key]
    name = cfg["name"]
    print(f"\n{'='*60}")
    print(f"  ABLATION {variant_key}: {name}")
    print(f"  swiglu={cfg['use_swiglu']} rel_bias={cfg['use_rel_bias']} attn_pol={cfg['use_attn_policy']}")
    print(f"  steps={max_steps} bs={batch_size} lr={lr}")
    print(f"{'='*60}\n")

    torch.manual_seed(seed)

    # Build model
    model = MicroChessTransformer(
        d_model=256, n_layers=4, n_heads=4, ffn_ratio=4,
        dropout=0.05, policy_head_dim=128,
        use_swiglu=cfg["use_swiglu"],
        use_rel_bias=cfg["use_rel_bias"],
        use_attn_policy=cfg["use_attn_policy"],
    ).to(DEVICE)

    n_params = sum(p.numel() for p in model.parameters())
    print(f"  Parameters: {n_params:,}")

    optimizer = AdamW(model.parameters(), lr=lr, weight_decay=0.01)
    scaler = GradScaler()

    # Load data
    print("  Loading shard 0...")
    train_data = load_shard(0)
    n_train = train_data["board_array"].shape[0]
    print(f"  Training positions: {n_train:,}")

    print("  Loading eval data...")
    eval_data = load_shard(1)  # Use shard 1 for eval (not seen during training)

    # Training loop
    results = {"variant": variant_key, "name": name, "params": n_params, "evals": []}
    step = 0
    t0 = time.time()
    epoch = 0

    while step < max_steps:
        perm = torch.randperm(n_train)
        for i in range(0, n_train - batch_size, batch_size):
            if step >= max_steps:
                break

            indices = perm[i:i+batch_size]
            board_input, targets, wdl, valid = prepare_batch(train_data, indices, DEVICE)

            if targets.shape[0] < 2:
                continue

            optimizer.zero_grad(set_to_none=True)
            with autocast("cuda", dtype=torch.float16):
                out = model(board_input)
                # Only compute policy loss on valid (remappable) moves
                policy_logits = out["policy_logits"][valid]
                p_loss = F.cross_entropy(policy_logits, targets)
                v_loss = F.cross_entropy(out["value_logits"][valid], wdl)
                loss = p_loss + 0.5 * v_loss

            scaler.scale(loss).backward()
            scaler.unscale_(optimizer)
            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            scaler.step(optimizer)
            scaler.update()

            step += 1

            if step % 100 == 0:
                elapsed = time.time() - t0
                pos_per_sec = step * batch_size / elapsed
                print(f"  step={step:4d} p_loss={p_loss.item():.3f} "
                      f"v_loss={v_loss.item():.3f} "
                      f"{pos_per_sec:.0f} pos/s")

            if step % eval_every == 0 or step == max_steps:
                ev = evaluate(model, eval_data, DEVICE, n_eval=5000)
                results["evals"].append({"step": step, **ev})
                print(f"  >>> EVAL step={step}: top1={ev['top1']:.4f} "
                      f"top3={ev['top3']:.4f} loss={ev['loss']:.3f}")

        epoch += 1

    elapsed = time.time() - t0
    results["total_time_s"] = elapsed
    results["final_pos_per_s"] = max_steps * batch_size / elapsed
    print(f"\n  Completed {name} in {elapsed:.1f}s ({results['final_pos_per_s']:.0f} pos/s)")

    # Cleanup
    del model, optimizer, scaler
    gc.collect()
    torch.cuda.empty_cache()

    return results


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--variant", type=str, default=None,
                       help="Run only this variant (A/B/C/D/E)")
    parser.add_argument("--steps", type=int, default=3000,
                       help="Training steps per variant")
    parser.add_argument("--eval-every", type=int, default=500)
    parser.add_argument("--batch-size", type=int, default=96)
    parser.add_argument("--lr", type=float, default=3e-4)
    parser.add_argument("--seed", type=int, default=42)
    args = parser.parse_args()

    print(f"Device: {DEVICE}")
    if torch.cuda.is_available():
        print(f"GPU: {torch.cuda.get_device_name()}")
        print(f"VRAM: {torch.cuda.get_device_properties(0).total_memory / 1e9:.1f} GB")
    print(f"Compact vocab size: {VOCAB_SIZE}")

    variants = [args.variant] if args.variant else list(VARIANTS.keys())
    all_results = {}

    for v in variants:
        res = train_variant(v, max_steps=args.steps, eval_every=args.eval_every,
                           batch_size=args.batch_size, lr=args.lr, seed=args.seed)
        all_results[v] = res

    # Summary table
    print(f"\n{'='*80}")
    print(f"  MICRO-ABLATION SUMMARY (step {args.steps})")
    print(f"{'='*80}")
    print(f"{'Variant':<12} {'Name':<12} {'Params':>8} {'Top-1':>8} {'Top-3':>8} "
          f"{'Loss':>8} {'Time(s)':>8} {'pos/s':>8}")
    print("-" * 80)

    for v in variants:
        r = all_results[v]
        final = r["evals"][-1] if r["evals"] else {}
        print(f"{v:<12} {r['name']:<12} {r['params']:>8,} "
              f"{final.get('top1', 0):>7.2%} {final.get('top3', 0):>7.2%} "
              f"{final.get('loss', 0):>8.3f} {r['total_time_s']:>7.1f}s "
              f"{r['final_pos_per_s']:>7.0f}")

    # Save results
    out_path = ROOT / "outputs" / "exp169_micro_ablation_results.json"
    with open(out_path, "w") as f:
        json.dump(all_results, f, indent=2)
    print(f"\nResults saved to {out_path}")


if __name__ == "__main__":
    main()
