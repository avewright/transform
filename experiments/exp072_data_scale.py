"""exp072: Data scaling — 10M positions × 2 epochs.

Hypothesis: Training on 10M positions for 2 epochs will outperform 2M × 6 epochs
(exp071, 22.9%) because the model was overfitting by epoch 5-6 with only 2M unique
positions. More diverse data should improve generalization.

Compute budget: Roughly matched — 10M × 2 = 20M position-passes vs 2M × 6 = 12M.
Slightly more compute but the key variable is data diversity, not total compute.

Architecture: Same as exp070/071 (12L/512d FusedEncoder, 38.5M params)
Data: 10M positions from avewright/chess-positions-lichess-sf (cached)
Epochs: 2
Seed: 42
"""
...
OUTPUT_DIR = Path("outputs/exp072_data_scale")
...
def cross_evaluate_hf(model, device):
    """Evaluate on the avewright/chess-positions test set for comparison."""
    from chess_features import batch_boards_to_fused_token_ids
    print("\n  Cross-evaluating on avewright/chess-positions HF test set...")
    try:
        from hf_data import load_eval_set
        hf_eval = load_eval_set(n=2500, split="test")
        if not hf_eval:
...
def main():
    ...
    print(f"{'='*70}")
    print(f" EXP072: DATA SCALE — 10M positions × 2 epochs")
    print(f"{'='*70}")
    ...
    print(f"  Seed: {SEED}")
    print(f"  Baseline: exp071 = 22.9% top-1 (6 epochs × 2M)")
    print()
    ...
    print(f"\n  vs exp071 (6 epochs × 2M): {best_acc - 0.2288:+.1%}")

    summary = {
        "experiment": "exp072_data_scale",
        "hypothesis": "10M diverse positions × 2 epochs beats 2M × 6 epochs",
