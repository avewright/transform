"""exp071: Extended training + cross-evaluation on old eval set.

Hypothesis: Training for 6 epochs instead of 2 on the same 2M positions will
significantly improve accuracy, since exp070 showed loss still decreasing at
epoch 2 end (policy loss 2.74, value loss 0.19 — no plateau).

Additionally: Cross-evaluate on the old avewright/chess-positions HF eval set
(2500 positions) to enable fair comparison with prior experiments (exp052-053).

This tests whether the lower exp070 accuracy (20.9% vs 35%+ prior) is due to:
  a) Harder eval set (different position distribution), OR
  b) Model actually being worse

If cross-eval shows >35% on the old eval set, it confirms (a) and validates
the approach. If not, the lichess-sf data distribution needs investigation.

Architecture: Same as exp070 (12L/512d FusedEncoder, 38.5M params)
Data: Same 2M positions from avewright/chess-positions-lichess-sf (cached)
Change: 6 epochs instead of 2
Seed: 42
"""
...
OUTPUT_DIR = Path("outputs/exp071_extended_training")
...
EPOCHS = 6               # 3x exp070
LR = 3e-4
WARMUP_FRAC = 0.03       # smaller warmup fraction with more epochs
VALUE_WEIGHT = 0.5
GRAD_CLIP = 1.0
SEED = 42

# Model dims (same as exp070)
ENCODER_DIM = 256
HIDDEN_DIM = 512
NUM_LAYERS = 12
NUM_HEADS = 8
...
def evaluate(model, eval_data, eval_tensors, device, encoder_type="fused", batch_size=128):
    model.eval()
...
# ── Cross-evaluation on old HF dataset ──

def cross_evaluate_hf(model, device):
    """Evaluate on the avewright/chess-positions test set for comparison with prior experiments."""
    from chess_features import board_to_fused_token_ids
    print("\n  Cross-evaluating on avewright/chess-positions HF test set...")
    try:
        from hf_data import load_eval_set
        hf_eval = load_eval_set(n=2500, split="test", repo_id="avewright/chess-positions")
        if not hf_eval:
            print("  WARNING: Could not load HF eval set")
            return None
    except Exception as e:
        print(f"  WARNING: HF eval failed: {e}")
        return None
    ...
    with torch.no_grad():
        for d in hf_eval:
            ...
            # Encode board for FusedBoardEncoder
            from chess_features import batch_boards_to_fused_token_ids
            batch_input = batch_boards_to_fused_token_ids([board], device)
            ...
            if pred_idx == true_idx:
                correct += 1
            topk = logits.topk(min(3, logits.shape[0])).indices.tolist()
            if true_idx in topk:
                top3_correct += 1
            total += 1
    ...
    result = {
        ...
        "top3_accuracy": top3_correct / max(total, 1),
        "n_eval": total,
        "eval_set": "avewright/chess-positions test",
    }
    print(f"  HF cross-eval: acc={result['accuracy']:.1%} top3={result['top3_accuracy']:.1%} (n={total})")
    return result
...
def train(train_tensors, eval_data, eval_tensors, device):
    ...
    # Cross-evaluate on old HF eval set
    hf_result = cross_evaluate_hf(model, device)

    return results_log, total_time, n_params, best_acc, hf_result
...
def main():
    ...
    print(f"{'='*70}")
    print(f" EXP071: EXTENDED TRAINING (6 epochs) + CROSS-EVAL")
    print(f"{'='*70}")
    ...
    print(f"  Seed: {SEED}")
    print(f"  Baseline: exp070 = 20.9% top-1 (2 epochs)")
    print()
    ...
    # Train
    results_log, total_time, n_params, best_acc, hf_result = train(
    ...
    if results_log:
        final = results_log[-1]
        print(f"  Final (lichess-sf eval): acc={final['accuracy']:.1%} "
              f"top3={final['top3_accuracy']:.1%} "
              f"sf_rank={final['mean_sf_rank']:.1f} val={final['value_accuracy']:.1%}")

    if hf_result:
        print(f"  Cross-eval (HF chess-positions): acc={hf_result['accuracy']:.1%} "
              f"top3={hf_result['top3_accuracy']:.1%}")

    print(f"\n  vs exp070 (2 epochs): {best_acc - 0.2088:+.1%}")

    # Save summary
    summary = {
        "experiment": "exp071_extended_training",
        "hypothesis": "6 epochs improves over 2 epochs since loss was still decreasing",
        ...
        "seed": SEED,
        "baseline_exp070": 0.2088,
        "hf_cross_eval": hf_result,
