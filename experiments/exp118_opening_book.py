"""exp118: Evaluate opening book + blend strategy.

Tests whether adding an opening book for the first ~10 moves improves ELO.
Compares:
  - baseline_greedy (no book)
  - baseline_blend (no book) 
  - book_greedy (book + greedy after book)
  - book_blend (book + blend after book)
"""
...
@torch.no_grad()
def strategy_greedy(model, board, device, use_book=False, **kw):
    if use_book:
...
@torch.no_grad()
def strategy_blend(model, board, device, top_k=10, value_weight=0.3, use_book=False, **kw):
    if use_book:
        bm = get_book_move(board)
        if bm is not None:
            return bm
    ...
    while not board.is_game_over(claim_draw=True) and len(board.move_stack) < ply_cap:
        if board.turn == color:
            ...
            if tb is not None:
                ...
                # Track if this was a book move
                bm = get_book_move(board)
                if bm is not None and move == bm:
                    book_moves += 1
        else:
...
def run_eval(model, strategy_fn, name, sf_elos, openings, games_per=2):
    ...
    for sf_elo in sf_elos:
        ...
        avg_book = sum(r["book_moves"] for r in results) / games if games else 0

        summary = {"sf_elo": sf_elo, "games": games, "score": sc, "w": w, "d": d, "l": l,
                    "avg_book_moves": round(avg_book, 1)}
        summaries.append(summary)
        log(f"  → {sc:.3f} ({w}W-{d}D-{l}L) avg_book={avg_book:.1f}")

    return {"strategy": name, "summaries": summaries}
...
def main():
    ...
    LOG_FILE = output_dir / "elo_eval_exp118_opening_book.log"
    json_out = output_dir / "elo_eval_exp118_opening_book.json"

    init_syzygy()

    # Use exp116b if available, else baseline
    if args.checkpoint:
        ckpt = args.checkpoint
    else:
        exp116b = ROOT / "outputs" / "exp116b_low_lr" / "best_model.pt"
        baseline = ROOT / "outputs" / "hf_checkpoint" / "best_model.pt"
        ckpt = exp116b if exp116b.exists() else baseline
    ...
    strategies = [
        ("greedy", strategy_greedy, {"use_book": False}),
        ("book_greedy", strategy_greedy, {"use_book": True}),
        ("blend_k10_w30", strategy_blend, {"top_k": 10, "value_weight": 0.30, "use_book": False}),
        ("book_blend_k10_w30", strategy_blend, {"top_k": 10, "value_weight": 0.30, "use_book": True}),
    ]
    ...
    for name, r in results.items():
        scores = " | ".join(
            f"SF{s['sf_elo']}:{s['score']:.3f}({s['w']}W-{s['d']}D-{s['l']}L,book={s['avg_book_moves']})"
            for s in r["summaries"]
        )
        log(f"  {name:30s} {scores}")
