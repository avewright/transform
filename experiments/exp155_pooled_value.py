"""exp155: Pooled value head — richer value input + higher value weight.

Hypothesis:
  The current value head reads ONLY the CLS token (1024-dim), which may be a
  lossy bottleneck. By pooling information from all 64 square tokens AND the CLS
  token (2048-dim input), the value head gets a much richer view of the position.
  Combined with higher value_weight (1.0 vs 0.5), this directly attacks the
  primary ELO bottleneck: value head quality.

  The ELO gauntlet showed exp149's value head at 55% middlegame accuracy —
  near random for 3-class WDL. At low MCTS sims (100), value quality dominates
  policy quality for ELO. The pooled head adds 0.65M params (204.0→204.7M) and
  gives the value head access to local piece interactions that CLS may compress away.

Expected gain: +3-8% value accuracy → +100-300 ELO at 100 sims.

Baseline: exp153 (hflip, cls value head, vw=0.5)
Control:  Same data, hflip, but cls value head with vw=0.5
Variable: Pooled value head + value_weight=1.0

Architecture: 204.7M (16L/1024d/16H, PooledValueHead)
  Value input: CLS (1024) + mean(64 squares) (1024) → 2048-dim
  Value MLP: Linear(2048,512) → ReLU → Linear(512,256) → ReLU → Linear(256,3)
Data: same 10.1M positions, hflip=True
LR: resume cosine schedule from exp149 epoch 1 position

Usage:
  python experiments/exp155_pooled_value.py
  python experiments/exp155_pooled_value.py --resume
  python experiments/exp155_pooled_value.py --eval-only --checkpoint outputs/exp155_pooled_value/best_model.pt
"""
...
OUTPUT_DIR = ROOT / "outputs" / "exp155_pooled_value"
...
# Key change: pooled value head
MODEL_CONFIG = ChessTransformerConfig(value_head_type="pool")
...
def main():
    ...
    ap.add_argument("--value-weight", type=float, default=1.0,
                    help="Value loss weight (default: 1.0, doubled from exp153's 0.5)")
    ...
    log("=" * 60)
    log("exp155: Pooled value head + hflip + value_weight=1.0")
    log(f"  device: {DEVICE}")
    log(f"  config: {MODEL_CONFIG}")

    # Build model with pooled value head
    model = build_model(MODEL_CONFIG)
    n_params = count_parameters(model)
    log(f"  params: {n_params/1e6:.1f}M (includes pooled value head)")
    ...
    if args.resume and resume_path.exists():
        ...
        # Load with strict=False: old value_head.* keys will be skipped,
        # new pooled value head (value_head.mlp.*) will be randomly initialized
        missing, unexpected = model.load_state_dict(sd, strict=False)
        log(f"  Loaded exp149 epoch_1 (strict=False)")
        log(f"    Missing keys (new pooled value head): {len(missing)}")
        log(f"    Unexpected keys (old cls value head): {len(unexpected)}")
        for k in missing:
            log(f"      + {k}")
        for k in unexpected:
            log(f"      - {k}")
        ...
        best_acc = ckpt.get("best_acc", 0.0)
        log(f"  Starting from step={start_step}, best_acc={best_acc:.2%}")
        log(f"  Pooled value head initialized RANDOMLY — will train from scratch")
        log(f"  Continuing with hflip + value_weight={args.value_weight}")

    model.to(DEVICE)
    model.train()

    # Optimizer — use higher LR for new value head parameters
    trunk_params = []
    value_head_params = []
    ...
    optimizer = AdamW([
        {"params": trunk_params, "lr": args.lr, "weight_decay": args.weight_decay},
        {"params": value_head_params, "lr": args.lr * 5, "weight_decay": args.weight_decay,
         "name": "value_head"},
    ], betas=(0.9, 0.95))
    ...
    if args.resume and resume_path.exists():
        ...
        # Load optimizer state from source for trunk params (not value head)
        source = Path(args.source_ckpt)
        ckpt = torch.load(source, map_location="cpu", weights_only=False)
        # Don't load optimizer state — param groups changed (2 groups vs 1)
        # Fresh optimizer for both groups is fine; trunk will adapt quickly
        log(f"  Fresh optimizer (param groups changed for pooled value head)")
    ...
    total_steps_full = steps_per_epoch * 3  # exp149's 3-epoch schedule
    ...
    log(f"  hflip=True, value_weight={args.value_weight}")
    log(f"  Pooled value head: CLS (1024) + mean(64 squares) (1024) → 2048-dim")
    ...
    with open(config_path, "w") as f:
        json.dump({"model": MODEL_CONFIG.to_dict(), "training": {
            ...
            "value_head": "pooled (CLS + mean squares)",
            "value_head_lr_multiplier": 5,
    ...
    # Initial eval (value should be bad — random pooled head)
    if eval_data:
        torch.cuda.empty_cache()
        acc, top3, top5, val_acc = run_eval(model, eval_data, eval_tensors)
        log(f"  BASELINE (new pool head): acc={acc:.2%} top3={top3:.2%} top5={top5:.2%} val={val_acc:.2%}")
    ...
    log(f"  LR={args.lr} (trunk), {args.lr*5} (value head)")
    ...
    log(f"  HFLIP=True, POOLED VALUE HEAD")
    log(f"{'='*60}")
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        log(f"  Starting epoch {epoch+1} (hflip + pooled value head)")
        ...
        for batch_input, move_targets, wdl_targets in loader:
            ...
            if accum_count >= args.accum_steps:
                ...
                for pg in optimizer.param_groups:
                    base_lr = lr
                    if pg.get("name") == "value_head":
                        base_lr = lr * 5  # Higher LR for new value head
                    pg["lr"] = base_lr
