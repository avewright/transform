"""exp135: Simulation scaling and c_puct exploration.

KEY FINDING from exp134: Noise and inner_temp have NO measurable effect.
Both noise=0.25 and noise=0.0 → 0.422 at 32 games. exp125 was a fluke.

True baseline: ~0.422 (1845 ELO) at c=2.5, 100 sims vs SF1900.

Now testing the two most promising levers:
  1. More simulations (200, 400): deeper search may overcome tactical blindspots
  2. Higher c_puct (3.5): even more exploration with weak policy
  3. Combined: 200 sims + higher c_puct

All tests at 32 games for statistical power. No noise (saves 25% of prior
spent on noise moves, identical performance). Simple is better.

Config matrix:
  A: sims_200     c=2.5, 200s, noise=0.0, fpu=0.25
  B: sims_400     c=2.5, 400s, noise=0.0, fpu=0.25
  C: cpuct_3.5    c=3.5, 100s, noise=0.0, fpu=0.25
  D: combo_200_35 c=3.5, 200s, noise=0.0, fpu=0.25
"""
...
CONFIGS = {
    "A": {
        "label": "sims_200",
        "sims": 200, "c_puct": 2.5, "noise_frac": 0.0, "fpu_reduction": 0.25,
    },
    "B": {
        "label": "sims_400",
        "sims": 400, "c_puct": 2.5, "noise_frac": 0.0, "fpu_reduction": 0.25,
    },
    "C": {
        "label": "cpuct_3.5",
        "sims": 100, "c_puct": 3.5, "noise_frac": 0.0, "fpu_reduction": 0.25,
    },
    "D": {
        "label": "combo_200_c35",
        "sims": 200, "c_puct": 3.5, "noise_frac": 0.0, "fpu_reduction": 0.25,
    },
}
...
def main():
    ...
    global LOG_PATH
    LOG_PATH = ROOT / "outputs" / "exp135_sim_scaling.log"
    json_path = ROOT / "outputs" / "exp135_sim_scaling.json"
    ...
    configs_to_run = (list(CONFIGS.keys()) if args.config == "all"
                      else [c.strip().upper() for c in args.config.split(",")])
    ...
    log("\nReferences:")
    log("  exp133 c=2.5 100s noise=0.0: 0.422 (8W-11D-13L) ~1845 (32 games)")
    log("  exp134A c=2.5 100s noise=0.25: 0.422 (11W-5D-16L) ~1845 (32 games)")
