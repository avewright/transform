"""exp143: Train 204M model on 10M positions — lower learning rate.

exp142 with LR=2e-5 showed accuracy declining after warmup phase:
  - Step 0 (baseline): 12.84%
  - Step 500 (LR~1e-5): 13.58% ← best
  - Step 1000 (LR=2e-5): 13.02% ← declining
  - Step 1500 (LR=2e-5): 12.96% ← back to baseline

Root cause: LR=2e-5 too aggressive for fine-tuning pre-trained 204M model.
The model improved during warmup (LR < 1e-5) but regressed at peak.

Fix: Lower peak LR to 1e-5, start from exp142 best checkpoint (step 500).

Usage:
  python experiments/exp143_204m_lowlr.py --epochs 3
  python experiments/exp143_204m_lowlr.py --resume
"""
...
OUTPUT_DIR = ROOT / "outputs" / "exp143_204m_lowlr"
...
# Starting checkpoint: exp142 best (step 500, 13.58% accuracy)
BASE_CKPT = ROOT / "outputs" / "exp142_204m_10m" / "best_model.pt"
# Fallback to exp137 if exp142 best doesn't exist
FALLBACK_CKPT = ROOT / "outputs" / "exp137_finetune_204m" / "final_model.pt"
...
def load_eval_data(eval_path):
    """Load eval.pt, reconstruct FENs from board_array."""
    import chess
...
def run_eval(model, eval_data, eval_tensors, batch_size=32):
    """Quick eval: top-1, top-3, value accuracy."""
    model.eval()
...
def main():
    ...
    ap.add_argument("--lr", type=float, default=1e-5)  # Halved from exp142's 2e-5
    ...
    ap.add_argument("--from-scratch", action="store_true",
                    help="Train from random init instead of checkpoint")
    args = ap.parse_args()
    ...
    log("=" * 60)
    log("exp143: 204M model on 10M positions (LR=1e-5)")
    log(f"  device: {DEVICE}")
    ...
    # Resume or load base checkpoint
    start_step = 0
    start_epoch = 0
    best_acc = 0.0
    ...
    if args.resume and resume_path.exists():
        ...
        log(f"  Resumed from latest.pt: step={start_step}, epoch={start_epoch}, best_acc={best_acc:.2%}")
    elif not args.from_scratch:
        # Try exp142 best first, then fallback to exp137
        ckpt_path = BASE_CKPT if BASE_CKPT.exists() else FALLBACK_CKPT
        if ckpt_path.exists():
        ...
        log(f"  Training from random initialization")

    model.to(DEVICE)
    model.train()

    # Fresh optimizer (don't inherit exp142's momentum which was at too-high LR)
    ...
    if not config_path.exists():
        with open(config_path, "w") as f:
            json.dump({"model": MODEL_CONFIG.to_dict(), "training": {
                ...
                "base_ckpt": str(BASE_CKPT.name),
                "note": "Lower LR than exp142 (1e-5 vs 2e-5). Started from exp142 best_model.pt (step 500, 13.58%)",
            }}, f, indent=2)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_input, move_targets, wdl_targets in loader:
            ...
            if torch.isnan(p_loss) or torch.isnan(v_loss):
                log(f"NaN detected at step {step}! Aborting.")
                save_checkpoint(model, optimizer, scaler, step, epoch, best_acc,
            ...
            if accum_count >= args.accum_steps:
                ...
                # Save (BEFORE eval to prevent data loss on OOM)
    ...
    log(f"\n{'='*60}")
    log(f"Training complete. Best accuracy: {best_acc:.2%}")
    log(f"{'='*60}")
