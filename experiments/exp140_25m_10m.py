"""exp140: Train 25M model from scratch on 10M positions.

Root cause: The 204M model is too slow to train on RTX 4060 (5 pos/s, 21 days).
Instead, train a 25.9M model (8L/512d/8H) from scratch on 10M positions.

Key insight from exp136:
  - 25M at 500K -> 18.5% acc, 1430 ELO (100 sims)
  - 204M at 224K -> 17.2% acc, 1845 ELO (100 sims)
  - Model depth helps MCTS, BUT data volume is the real bottleneck

Strategy:
  - Train 25M on 10M positions (20x more data than exp136)
  - Expected policy: 30-50% top-1 (vs 18.5%)
  - Then use MCTS with 500-1000 sims (12x faster than 204M)
  - Better policy + deeper search should exceed 1845 ELO

Hardware: RTX 4060 8GB
  - bs=64 at 316 pos/s (measured in exp136)
  - 10M positions x 3 epochs = 30M instances
  - ETA: ~26 hours

Usage:
  python experiments/exp140_25m_10m.py --epochs 3
"""
...
OUTPUT_DIR = ROOT / "outputs" / "exp140_25m_10m"
SHARD_DIR = ROOT / "outputs" / "exp139_massive_train" / "shards"
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
LOG_PATH = None
SHUTDOWN = False

# Model config: 25.9M params (8L/512d/8H)
...
def run_eval(model, eval_data, eval_tensors, batch_size=256):
...
def main():
    ...
    log("=" * 60)
    log("exp140: 25M model on 10M positions")
    log(f"  device: {DEVICE}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if eval_data:
            torch.cuda.empty_cache()
            acc, top3, val_acc = run_eval(model, eval_data, eval_tensors, batch_size=64)
