"""exp142: Train 204M model on 10M positions.

After the SpatialPolicyHead optimization (project-then-gather), 204M training
is now feasible at ~75 pos/s (up from 5 pos/s).

Starting from exp137 checkpoint (204M fine-tuned on 224K + 32K = 256K positions).
Training on 10M positions (40x more data).

Key findings motivating this:
  - 25M model: 0/8 vs SF1320 greedy (~920 ELO) - model depth is essential
  - 204M model: ~1845 ELO on only 224K positions
  - Ruoss et al. 2024: 270M on 10M → 2895 ELO without search
  - Policy head optim: 15x training speedup, no accuracy change

Hardware: RTX 4060 8GB
  - bs=24, no grad_ckpt: ~74 pos/s, 5194 MB peak
  - 10M positions × 3 epochs ÷ 98 pos/s ≈ 86 hours (3.6 days)

LR note: 1e-4 causes NaN at step ~3300 (peak LR after warmup).
  Use 2e-5 for stable fine-tuning.

Usage:
  python experiments/exp142_204m_10m.py --epochs 3
  python experiments/exp142_204m_10m.py --resume
"""
...
OUTPUT_DIR = ROOT / "outputs" / "exp142_204m_10m"
SHARD_DIR = ROOT / "outputs" / "exp139_massive_train" / "shards"
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
LOG_PATH = None
SHUTDOWN = False

# Use the default 204M config
MODEL_CONFIG = DEFAULT_200M_CONFIG

# Starting checkpoint: exp137 fine-tuned 204M
BASE_CKPT = ROOT / "outputs" / "exp137_finetune_204m" / "final_model.pt"
...
def main():
    ...
    ap.add_argument("--from-scratch", action="store_true",
                    help="Train from random init instead of exp137 checkpoint")
    args = ap.parse_args()
    ...
    log("=" * 60)
    log("exp142: 204M model on 10M positions")
    log(f"  device: {DEVICE}")
    ...
    elif not args.from_scratch and BASE_CKPT.exists():
        ckpt = torch.load(BASE_CKPT, map_location="cpu", weights_only=False)
        sd = ckpt.get("model_state_dict", ckpt)
        sd = {k.replace("_orig_mod.", ""): v for k, v in sd.items()}
        model.load_state_dict(sd, strict=False)
        log(f"  Loaded base checkpoint: {BASE_CKPT.name}")
    elif args.from_scratch:
        log(f"  Training from random initialization")
    else:
        log(f"  WARNING: No base checkpoint found at {BASE_CKPT}, training from scratch")
    ...
    log(f"ETA: ~{total_pos * args.epochs / 74 / 3600:.0f} hours at 74 pos/s")
    log(f"{'='*60}")
