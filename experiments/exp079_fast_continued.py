"""exp079: Fast continued pretraining from local cached data + soft targets.

Hypothesis: Training the 200M model on 200K new positions (depth-22 Stockfish labels
from lichess-sf) with proper anti-forgetting measures (replay mixing + low LR) will
improve accuracy beyond the 46.7% baseline.

Key differences from exp078:
  - Reads from LOCAL cached data (data/lichess_sf_cached_200k.jsonl) → ~100x faster
  - Full 200K positions loaded into memory for instant random access
  - Soft targets where multi-PV data is available
  - Combined with 47.5K original training data as replay buffer
  - Multiple epochs over the data with reshuffling
...
# ---- Paths ----
OUTPUT_DIR = Path("outputs/exp079_fast_continued")
...
def compute_loss(model, boards, targets, soft_targets, wdl_targets, device):
    """Combined CE + soft KL + WDL value loss."""
    ...
    if has_soft:
        ...
        kl_loss = F.kl_div(log_probs, soft_batch, reduction="batchmean")
...
def make_epoch_batches(new_data, replay_data, batch_size=TRAIN_BATCH):
    """Create shuffled batches for one epoch, mixing new + replay data."""
...
def main():
    ...
    LOG_FILE = str(OUTPUT_DIR / "exp079.log")

    log("=" * 70)
    log("exp079: Fast continued pretraining (local cached data + soft targets)")
    log("=" * 70)
    log(f"Device: {DEVICE}")
    ...
    # Save results JSON
    results = {
        "experiment": "exp079_fast_continued",
