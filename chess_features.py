...
# Fused piece-color tokens: 13 types
...
# --- Fused piece-color tokenization ---

def board_to_fused_token_ids(board: chess.Board) -> dict[str, torch.Tensor]:
    """Convert a chess.Board to fused piece-color token IDs.

    Returns dict with:
        fused_ids:  (64,) long — 0=empty, 1-6=white P/N/B/R/Q/K, 7-12=black
        turn:       (1,)  long — 0=white, 1=black
        castling:   (1,)  long — 0-15 (4-bit)
        ep_file:    (1,)  long — 0=none, 1-8=file a-h
    """
    fused_ids = torch.zeros(64, dtype=torch.long)

    for sq, piece in board.piece_map().items():
        fused_ids[sq] = _FUSED_PIECE_IDX[(piece.piece_type, piece.color)]
...
def batch_boards_to_fused_token_ids(
    ...
    """Convert a list of boards to batched fused token IDs."""
    batch = [board_to_fused_token_ids(b) for b in boards]
    result = {
