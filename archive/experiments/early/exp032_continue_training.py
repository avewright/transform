"""exp032: Continue training from exp031 checkpoint with low LR.

Hypothesis: Fine-tuning the 51.2% model (exp031, 6 epochs on 460K) for 4 more
epochs with a low constant learning rate will push accuracy to ~53%, since the
loss was still declining at 2.06.

Background:
  - exp031: 8L/512d, 460K×6ep → 51.2%, loss=2.06 (still declining)
  - The cosine LR hit ~0 at epoch 6, so no more learning happened
  - A fresh optimizer with low constant LR (1e-5) acts as careful fine-tuning
  - Empirically, this "warm restart" technique often works when LR reaches zero

Design:
  - Load exp031 best checkpoint (epoch 6, 51.2%)
  - Continue training for 4 epochs with constant LR=1e-5
  - Same 460K data, same architecture
  - Saves best checkpoint for future use

Primary metric: top-1 accuracy (best across epochs)
Time budget: ~2 hours (4 epochs × ~27 min each + eval + games)
"""
...
OUTPUT_DIR = Path("outputs/exp032_continue_training")
CHECKPOINT_PATH = Path("outputs/exp031_extended_training/best_checkpoint.pt")
STOCKFISH_PATH = "stockfish/stockfish/stockfish-ubuntu-x86-64-avx2"

NUM_EVAL = 1000
EPOCHS = 4  # Additional epochs beyond exp031's 6
BATCH_SIZE = 128
ACCUM_STEPS = 2
LR = 1e-5  # Low constant LR for fine-tuning
ENCODER_DIM = 256
HIDDEN_DIM = 512
NUM_LAYERS = 8
NUM_HEADS = 8
...
# === Architecture (must match exp031 exactly) ===
...
def main():
    ...
    print(f"Experiment: exp032_continue_training")
    print(f"Hypothesis: Fine-tuning exp031 (51.2%) with low LR for 4 more epochs")

    # --- Load checkpoint ---
    print(f"\n[1/4] Loading exp031 checkpoint...")
    ...
    print(f"  Loaded from epoch {ckpt['epoch']}, accuracy {ckpt['accuracy']:.1%}")
    print(f"  Trainable: {model.trainable_params():,}")

    # --- Load data ---
    print(f"\n[2/4] Loading HuggingFace dataset...")
    ...
    # Pre-eval to verify checkpoint
    pre_ev = evaluate_accuracy(model, eval_data, device)
    print(f"  Pre-train eval: {pre_ev['accuracy']:.1%} (should be ~51.2%)")

    # --- Train ---
    print(f"\n[3/4] Continuing training for {EPOCHS} more epochs (LR={LR})")

    params = list(model.parameters())
    optimizer = AdamW(params, lr=LR, weight_decay=0.01)
    # Constant LR — no schedule needed for fine-tuning

    history = []
    best_acc = pre_ev["accuracy"]
    ...
    for epoch in range(EPOCHS):
        ...
        history.append({**ev, "loss": round(avg_loss, 4),
                       "epoch": effective_epoch})
        marker = " *BEST*" if ev["accuracy"] >= best_acc else ""
        print(f"  Epoch {effective_epoch} (+{epoch+1}): loss={avg_loss:.4f} "
              f"acc={ev['accuracy']:.1%} top3={ev['top3_accuracy']:.1%}"
              f"{marker} [{elapsed:.0f}s]")

    # --- Play games ---
    # Load best checkpoint if it was saved
    best_ckpt_path = OUTPUT_DIR / "best_checkpoint.pt"
    if best_ckpt_path.exists():
        ckpt2 = torch.load(best_ckpt_path, map_location=device, weights_only=True)
        model.load_state_dict(ckpt2["model_state"])
        print(f"\n  Loaded best checkpoint (epoch {ckpt2['epoch']}, acc={ckpt2['accuracy']:.1%})")
    ...
    results = {
        "experiment": "exp032_continue_training",
        "hypothesis": "Fine-tuning exp031 (51.2%) with constant LR=1e-5 for 4 more epochs",
        ...
        "training": {"additional_epochs": EPOCHS, "total_effective_epochs": 10,
                     "batch_size": BATCH_SIZE, "accum_steps": ACCUM_STEPS,
                     "lr": LR, "schedule": "constant"},
        "baseline": {"exp031_acc": 0.512, "pre_eval": pre_ev["accuracy"]},
        "results": {
            "best_acc": best_acc,
            "history": history,
        },
        ...
        "device": str(device),
        "command": "python experiments/exp032_continue_training.py",
    }
    with open(OUTPUT_DIR / "results.json", "w") as f:
        json.dump(results, f, indent=2)

    print(f"\n{'='*60}")
    print(f" SUMMARY: exp032_continue_training")
    print(f" Hypothesis: Continue exp031 (51.2%) with LR={LR}")
    print(f" Pre-eval: {pre_ev['accuracy']:.1%}")
    print(f" Best accuracy: {best_acc:.1%} (exp031: 51.2%)")
    print(f" Delta vs exp031: {best_acc - 0.512:+.1%}")
    print(f" Epoch progression:")
    for h in history:
        print(f"   Epoch {h['epoch']}: {h['accuracy']:.1%} acc, loss={h['loss']:.4f}")
